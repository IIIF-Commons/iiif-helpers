...
import type { LanguageMap as InternationalString } from '@iiif/parser/presentation-4/types';
import { describe, expect, test } from 'vitest';
import compositeTest from '../../fixtures/presentation-4/upgraded-from-p3/cookbook--composite.json';
import delftExample from '../../fixtures/presentation-4/upgraded-from-p3/exhibitions--novieten.json';
import { buildLocaleString, getAvailableLanguagesFromResource, getValue, iiifString } from '../../src/i18n';
...
describe('i18n helper', () => {
  describe('buildLocaleString()', () => {
    ...
    test('wont match partial, if strict', () => {
      expect(
        buildLocaleString({ 'en-GB': ['A'], none: ['A'] }, 'en', {
          strictFallback: true,
        })
      ).toEqual('A');
      expect(
        buildLocaleString({ 'en-US': ['A'], none: ['A'] }, 'en', {
          strictFallback: true,
        })
      ).toEqual('A');
    });
    ...
    test('it can match fallback languages', () => {
      expect(
        buildLocaleString({ en: ['A'], none: ['_'] }, 'cy-GB', {
          fallbackLanguages: ['en-GB'],
        })
      ).toEqual('A');
      expect(
        buildLocaleString({ en: ['_'], none: ['A'] }, 'cy-GB', {
          fallbackLanguages: [],
        })
      ).toEqual('A');
      expect(
        buildLocaleString({ en: ['_'], none: ['A'], '@none': ['_'] }, 'cy-GB', {
          fallbackLanguages: [],
        })
      ).toEqual('A');
      expect(
    ...
    test('multiple values with separators', () => {
      ...
      expect(
        buildLocaleString({ none: ['A', 'B', 'C', 'D', 'E'] }, 'none', {
          separator: ' ',
        })
      ).toEqual('A B C D E');
    });
